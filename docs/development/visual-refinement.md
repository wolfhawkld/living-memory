...
未建立时间起点的节点继续用空心环表达未知，颜色改为 `#6faef2`，弱光晕透明度由 0.08 调至 0.12。默认关系线由 `#668caf` 调至 `#709fce`；选中后非邻接边由 `#2b4563` 调至 `#3e5e80`，提高整体可见度。已有 recent/revisit/stale/pending 状态色、选中外环和高亮连线颜色保留；1px 原生线与现有 Bloom 开关继续生效。边的光晕仍受共用 Bloom 阈值控制，没有添加新的粗线几何体。
