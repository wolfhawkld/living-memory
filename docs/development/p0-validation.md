...
| 检查 | 结果 | 覆盖内容 |
| --- | --- | --- |
| `npm test` | 22/22 通过 | 9 项时间模型、5 项 KG 解析、8 项存储/API 测试 |
| `npm run build` | 通过 | 严格 TypeScript 检查与生产构建；3D 依赖包体约 1.66 MB（gzip 约 454 KB），保留体积提示 |
| 真实 KG 只读导入 | 通过 | 数学目录 87 个可选概念，本次选 20 个、15 条范围内关系；1 条解析诊断，不自动造边 |
| Chromium 端到端 | 4/4 通过 | 实际 WebGL、重温与模拟、页面重载、切换列表后重建图谱、回忆遮蔽与观察、补记与导出、断线重试 |

已检查最终实际界面截图：中文、状态颜色、节点与关系线正常显示；来源版本号不会撑破侧栏。以下为合成演示数据，重温状态由测试操作产生：

![P0 独立 Web 原型实际截图](images/p0-demo.png)

真实来源路径、正文、学习数据库和浏览器测试产物未纳入版本控制。测试使用合成知识与临时数据库；真实 KG 检查未写入来源文件。
