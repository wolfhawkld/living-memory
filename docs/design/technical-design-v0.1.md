...
### 2.3 初始遗忘估计与再访规则 `rules-v1`

这是面向开发和早期数据收集的**保守基线**，不是已经验证的个人遗忘曲线。`stage` 仅为再访档位，不能解释为记忆强度或遗忘机制。LM-003 先用样例轨迹审查规则，再实现 LM-007。
...
首先锁定记忆目标、证据资格、规则轨迹和可解释输出；再实现存储/接口及最小任务流程。图谱用相同 fixtures 并行开发，最后接真实投影。Obsidian 适配只消费这些接口，放到后续版本。
