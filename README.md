...
一个由记忆与遗忘机制驱动的个人知识与学习项目：通过动态知识图谱和学习任务，连接知识的外化、整合、回忆、使用与认知修正。目前处于需求、设计与开发任务拆分阶段。
...
首批材料使用用户自建的 **progressive-kg**：各领域中已经学过、部分可能已经会应用的概念。首版分别管理核心解释、细节/关系、场景提取与适用判断，区分查询接触、提示后恢复、即时纠错和延迟独立表现；学习反馈也推动知识来源、关系与个人理解的修正。
...
状态：2026-09-16，已保存首版需求、技术设计、18 项开发任务与 23 项验收规格；附一个离线 HTML/SVG 风格预览。应用尚未开发，真实知识库、3D 引擎、学习模型和语音链路尚未接通。
